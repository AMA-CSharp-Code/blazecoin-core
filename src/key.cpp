...
int ec_seckey_import_der(const secp256k1_context* ctx, unsigned char *out32, const unsigned char *seckey, size_t seckeylen) {
    ...
    ptrdiff_t len;
    if (*seckey & 0x80u) {
        /* long form: classic Bitcoin/OpenSSL keys serialized with explicit EC
           parameters are >127 bytes, so the SEQUENCE length uses long form. */
        ...
        len = seckey[lenb-1] | (lenb > 1 ? seckey[lenb-2] << 8 : 0u);
        seckey += lenb;
    } else {
        /* short form: Blazecoin V1.5 / 0.8.x wallets store the compact
           ECPrivateKey using the named-curve OID (not explicit parameters), so
           the SEQUENCE fits in <128 bytes and uses short-form length. Accept it
           so migratewallet can read these legacy wallets. The key is still
           validated by secp256k1_ec_seckey_verify below, so this only widens
           which valid DER encodings are parsed. */
        len = *seckey; seckey++;
    }
    if (end - seckey < len) {
        return 0;

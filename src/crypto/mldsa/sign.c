...
*
* Description: Generates public and private key from an explicit 32-byte
*              seed xi (FIPS 204 Algorithm 6, ML-DSA.KeyGen_internal).
*              Blazecoin: replaces the reference crypto_sign_keypair, which
*              drew the seed from randombytes(); callers derive xi
*              deterministically (PQ_SIGNATURES.md s6) and no RNG is linked.
*
* Arguments:   - uint8_t *pk: pointer to output public key (allocated
*                             array of CRYPTO_PUBLICKEYBYTES bytes)
*              - uint8_t *sk: pointer to output private key (allocated
*                             array of CRYPTO_SECRETKEYBYTES bytes)
*              - const uint8_t *seed: the 32-byte seed xi
*
* Returns 0 (success)
**************************************************/
int crypto_sign_keypair_from_seed(uint8_t *pk, uint8_t *sk, const uint8_t seed[SEEDBYTES]) {
  ...
  /* Expand the caller-supplied seed into rho, rhoprime and key */

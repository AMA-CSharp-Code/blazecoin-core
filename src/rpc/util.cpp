...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: application/json' http://127.0.0.1:55413/\n";
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-type: application/json' http://127.0.0.1:55413/\n";
}
...
public:
    ...
    {
        // Blazecoin P2PQH (PQ_SIGNATURES.md s5): validateaddress / getaddressinfo report the PQ type and algorithm.
        UniValue obj(UniValue::VOBJ);
        obj.pushKV("isscript", false);
        obj.pushKV("iswitness", false);
        obj.pushKV("type", "pqkh");
        obj.pushKV("algorithm", "ML-DSA-44");
        obj.pushKV("pqkh", HexStr(id));
        return obj;
    }
};

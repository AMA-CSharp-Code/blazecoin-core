...
const char * const BITCOIN_CONF_FILENAME = "blazecoin.conf";
...
{
    // Per-version data directory so V1.5 / V2.0 / production wallets can
    // coexist without corrupting each other's chainstate / wallet.dat.
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BlazecoinV2.0
    //   new: C:\Users\Username\AppData\Local\BlazecoinV2.0
    // macOS: ~/Library/Application Support/BlazecoinV2.0
    // Unix-like: ~/.blazecoinv2.0
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BlazecoinV2.0";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BlazecoinV2.0";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BlazecoinV2.0";
#else
    // Unix-like
    return pathRet / ".blazecoinv2.0";
#endif
#endif
}

...
/** Blazecoin P2PQH (PQ_SIGNATURES.md s6): the wallet's pqkh(mldsa44:<master_seed>/*) descriptor for one chain.
 *
 *  master_seed = TaggedHash("Blazecoin/PQ/master", k || chain)   i.e. SHA256(SHA256(tag) || SHA256(tag) || k || chain)
 *    k     = the wallet's 32-byte BIP32 master private key (the HD secret every other descriptor derives from)
 *    chain = 0x00 for the external (receive) chain, 0x01 for the internal (change) chain
 *  key i: xi_i = TaggedHash("Blazecoin/MLDSA44/seed", master_seed || uint32_le(i)); (pk_i, sk_i) = ML-DSA-44.KeyGen_internal(xi_i)
 *
 *  seed_key receives the seed as a CKey: that is the secret the descriptor SPKM stores (plaintext or
 *  encrypted) exactly like an xprv's root key, filed under its secp256k1 marker pubkey. */
WalletDescriptor GeneratePQWalletDescriptor(const CKey& master_key, bool internal, CKey& seed_key);
} // namespace wallet

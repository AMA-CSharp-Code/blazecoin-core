...
public:
    ...
    /** Blazecoin: sign a message with the ML-DSA-44 key behind a "BQ..." (P2PQH) address, see MessageSignPQ. */
    virtual SigningResult SignMessagePQ(const std::string& message, const PQKeyHash& pqkh, std::string& str_sig) const { return SigningResult::SIGNING_FAILED; };
...
public:
    ...
    bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors) const override;
    SigningResult SignMessage(const std::string& message, const PKHash& pkhash, std::string& str_sig) const override;
    SigningResult SignMessagePQ(const std::string& message, const PQKeyHash& pqkh, std::string& str_sig) const override;
    std::optional<common::PSBTError> FillPSBT(PartiallySignedTransaction& psbt, const PrecomputedTransactionData& txdata, int sighash_type = SIGHASH_DEFAULT, bool sign = true, bool bip32derivs = false, int* n_signed = nullptr, bool finalize = true) const override;

...
const std::string WALLETDESCRIPTORPQCACHE{"walletdescriptorpqcache"}; // Blazecoin: expanded ML-DSA-44 pubkeys of a pqkh() descriptor
...
bool WalletBatch::WriteDescriptorPQCache(const std::vector<unsigned char>& pubkey, const uint256& desc_id, uint32_t der_index)
{
...
bool WalletBatch::WriteDescriptorCacheItems(const uint256& desc_id, const DescriptorCache& cache)
{
    ...
    for (const auto& [der_index, pubkey] : cache.GetCachedPQPubKeys()) {
        if (!WriteDescriptorPQCache(pubkey, desc_id, der_index)) {
            return false;
        }
    }
    return true;
...
static DBErrors LoadDescriptorWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    LoadResult desc_res = LoadRecords(pwallet, batch, DBKeys::WALLETDESCRIPTOR,
        ...
        // Blazecoin: get the expanded ML-DSA-44 public keys of a pqkh() descriptor (PQ_SIGNATURES.md s6)

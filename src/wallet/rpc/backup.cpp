        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            if (OutputTypeFromDestination(dest) == OutputType::PQ) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Post-quantum (BQ) addresses cannot be imported into legacy wallets");
            }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Blazecoin: a "BQ..." address has an ML-DSA-44 key, which has no WIF form and lives only in a descriptor wallet.
    if (std::holds_alternative<PQKeyHash>(DecodeDestination(strAddress))) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Post-quantum (BQ) addresses have no exportable private key; use listdescriptors with private=true to back up the pqkh descriptor");
    }
...
static UniValue ProcessImportDescriptor(ImportData& import_data, std::map<CKeyID, CPubKey>& pubkey_map, std::map<CKeyID, CKey>& privkey_map, std::set<CScript>& script_pub_keys, bool& have_solving_data, const UniValue& data, std::vector<CKeyID>& ordered_pubkeys)
{
    ...
    if (parsed_desc->GetOutputType() == OutputType::PQ) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Post-quantum (pqkh) descriptors cannot be imported into legacy wallets");
    }

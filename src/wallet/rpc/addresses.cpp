...
RPCHelpMan getnewaddress()
{
    return RPCHelpMan{"getnewaddress",
                ...
                "so payments received with the address will be associated with 'label'.\n",
                {
                    ...
                    {"address_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -addresstype"}, "The address type to use. Options are \"legacy\", \"p2sh-segwit\", \"bech32\", \"bech32m\", and \"pq\" (Blazecoin post-quantum, BQ... address)."},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[1].isNull()) {
        ...
        } else if ((parsed.value() == OutputType::BECH32M || parsed.value() == OutputType::PQ) && pwallet->GetLegacyScriptPubKeyMan()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Legacy wallets cannot provide %s addresses", FormatOutputType(parsed.value())));
        }
...
RPCHelpMan getrawchangeaddress()
{
    return RPCHelpMan{"getrawchangeaddress",
                ...
                {
                    {"address_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The address type to use. Options are \"legacy\", \"p2sh-segwit\", \"bech32\", \"bech32m\", and \"pq\" (Blazecoin post-quantum, BQ... address)."},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[3].isNull()) {
        ...
        } else if (parsed.value() == OutputType::PQ) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "pq (post-quantum) addresses are single-key P2PQH outputs and cannot wrap a multisig script");
        }
        output_type = parsed.value();
    }
...
public:
    ...
    UniValue operator()(const PQKeyHash& id) const
    {
        // Blazecoin P2PQH: the ML-DSA-44 public key when this wallet knows it (DescribeAddress adds type/algorithm/pqkh).
        UniValue obj(UniValue::VOBJ);
        PQKeyPair key;
        if (provider && provider->GetPQKey(uint256(id), key) && !key.pubkey.empty()) {
            obj.pushKV("pqpubkey", HexStr(key.pubkey));
        }
        return obj;
    }
};
    ...
    return RPCHelpMan{"getaddressinfo",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::STR, "type", /*optional=*/true, "Blazecoin: \"pqkh\" for a post-quantum P2PQH address (PQ_SIGNATURES.md s5)"},
                        {RPCResult::Type::STR, "algorithm", /*optional=*/true, "Blazecoin: the PQ signature algorithm of a P2PQH address (\"ML-DSA-44\")"},
                        {RPCResult::Type::STR_HEX, "pqkh", /*optional=*/true, "Blazecoin: the 32-byte PQ key hash of a P2PQH address"},
                        {RPCResult::Type::STR_HEX, "pqpubkey", /*optional=*/true, "Blazecoin: the 1,312-byte ML-DSA-44 public key of a P2PQH address, when this wallet knows it"},

...
//! -fallbackfee default. Non-zero on Blazecoin: the chain has no fee market for
//! estimateSmartFee to learn from (near-empty zero-fee blocks), so with the upstream
//! default of 0 ("disabled") every fresh wallet's FIRST send fails with "Fee estimation
//! failed. Fallbackfee is disabled." 1000 sat/kvB matches DEFAULT_TRANSACTION_MINFEE
//! (~0.00000226 BLZ on a typical tx); -fallbackfee=0 still disables explicitly.
...
//! Default for -addresstype. Blazecoin: LEGACY (P2PKH, "B…"), NOT Bitcoin's bech32.
//! SegWit is consensus-disabled here (SegwitHeight = INT_MAX), so a bech32 "blz1…"
//! address is unspendable — sending funds to one loses them PERMANENTLY. Baking legacy
//! in means a conf-less install still hands out safe addresses by default (belt-and-
//! suspenders to the shipped blazecoin.conf's addresstype/changetype=legacy).
...
public:
    ...
    /** Blazecoin: sign a message with the ML-DSA-44 key of a "BQ..." (P2PQH) address (see MessageSignPQ). */
    SigningResult SignMessagePQ(const std::string& message, const PQKeyHash& pqkh, std::string& str_sig) const;

    /** Blazecoin: the height from which P2PQH outputs are standard and spendable (DEPLOYMENT_PQSIG). */
    int PQSigActivationHeight() const;
    /** Blazecoin: whether DEPLOYMENT_PQSIG is active for the block after the wallet's last processed block -
     *  the condition under which the mempool will accept a transaction paying to or spending a P2PQH output. */
    bool IsPQSigActiveForNextBlock() const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

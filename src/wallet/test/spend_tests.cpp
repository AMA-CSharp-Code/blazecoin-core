...
BOOST_FIXTURE_TEST_CASE(SubtractFee, TestChain100Setup)
{
    ...
    // Blazecoin: the wallet's only mature coin here is the block-1 premine coinbase
    // (20,650,000 BLZ), not upstream's flat 50-coin reward, so the recipient amount is
    // derived from it — the "recipient ≈ full input, so change is uneconomical" intent
    // needs the real input value.
    const CAmount input_value{m_coinbase_txns[0]->vout[0].nValue};
    auto check_tx = [&wallet, input_value](CAmount leftover_input_amount) {
        CRecipient recipient{PubKeyDestination({}), input_value - leftover_input_amount, /*subtract_fee=*/true};
...
BOOST_FIXTURE_TEST_CASE(wallet_duplicated_preset_inputs_test, TestChain100Setup)
{
    ...
    // Add 4 spendable UTXO to the wallet. Blazecoin: these mature the premine (20,650,000 BLZ)
    // plus three 13-BLZ coinbases rather than upstream's four 50-coin rewards, so the
    // over-target amount below is computed from the actual total instead of the hardcoded
    // 299-of-200 coins.
    ...
    // The tx target is set ~99 coins past everything the wallet can cover.
    CAmount total_available{0};
    for (const COutput& coin : coins) total_available += coin.txout.nValue;
    std::vector<CRecipient> recipients{{*Assert(wallet->GetNewDestination(OutputType::BECH32, "dummy")),
                                           /*nAmount=*/total_available + 99 * COIN, /*fSubtractFeeFromAmount=*/true}};
    CCoinControl coin_control;

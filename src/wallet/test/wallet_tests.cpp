...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        // Blazecoin: the scan picks up the oldTip and newTip coinbases — two 13-BLZ
        // early-subsidy rewards, not upstream's two 50-coin ones.
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, 2 * m_coinbase_txns.back()->vout[0].nValue);
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(result.last_scanned_block, newTip->GetBlockHash());
        BOOST_CHECK_EQUAL(*result.last_scanned_height, newTip->nHeight);
        // Blazecoin: only the newTip coinbase (13 BLZ) is scanned after pruning.
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, m_coinbase_txns.back()->vout[0].nValue);
    }
...
BOOST_FIXTURE_TEST_CASE(importwallet_rescan, TestChain100Setup)
{
    ...
    // were scanned, and no prior blocks were scanned.
    {
        ...
        // Blazecoin: 30-block fixture + the 3 blocks mined above (upstream: 100 + 3).
        BOOST_CHECK_EQUAL(m_coinbase_txns.size(), 33U);
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    wtx.MarkDirty();
    AddKey(wallet, coinbaseKey);
    // Blazecoin: wtx is the fixture's latest coinbase — a 13-BLZ early-subsidy reward.
...
BOOST_FIXTURE_TEST_CASE(ListCoinsTest, ListCoinsTestingSetup)
{
    ...
    // Blazecoin: the one mature coin is the block-1 premine (20,650,000 BLZ), not 50.
    BOOST_CHECK_EQUAL(m_coinbase_txns[0]->vout[0].nValue, WITH_LOCK(wallet->cs_wallet, return AvailableCoins(*wallet).GetTotalAmount()));
                     ...
                     std::map<OutputType, size_t>& expected_coins_sizes)
{
    ...
    for (const auto& [type, size] : expected_coins_sizes) BOOST_CHECK_MESSAGE(size == available_coins.coins[type].size(), strprintf("sent_type=%s checked_type=%s expected=%d got=%d", FormatOutputType(out_type), FormatOutputType(type), size, available_coins.coins[type].size()));
}
...
BOOST_FIXTURE_TEST_CASE(BasicOutputTypesTest, ListCoinsTest)
{
    ...
    //
    // Blazecoin: unlike upstream (where change matches the recipient's output type), this
    // fork's TransactionChangeType always returns LEGACY when the wallet's default address
    // type is legacy — the deliberate blz1-footgun guard (see DEFAULT_ADDRESS_TYPE in
    // wallet.h). Each self transfer therefore adds:
    //   1. One UTXO of the recipient's type
    //   2. One LEGACY UTXO from the change

    for (const auto& out_type : OUTPUT_TYPES) {
        if (out_type == OutputType::UNKNOWN) continue;
        // Blazecoin: P2PQH outputs are refused until DEPLOYMENT_PQSIG is active (never on this
        // mainnet-parameter test chain); the PQ bucket is exercised in pq_wallet_tests.
        if (out_type == OutputType::PQ) continue;
        expected_coins_sizes[out_type] += 1U;                // the recipient output
        expected_coins_sizes[OutputType::LEGACY] += 1U;      // the change is always legacy

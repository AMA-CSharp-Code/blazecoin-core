...
util::Result<CTxDestination> LegacyScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    ...
    assert(type != OutputType::BECH32M && type != OutputType::PQ); // Blazecoin: P2PQH is descriptor-wallet only
...
IsMineResult IsMineInner(const LegacyDataSPKM& keystore, const CScript& scriptPubKey, IsMineSigVersion sigversion, bool recurse_scripthash=true)
{
    ...
    case TxoutType::PQ_KEYHASH: // Blazecoin: legacy wallets hold no ML-DSA keys
        break;
...
bool DescriptorScriptPubKeyMan::SetupDescriptorGeneration(WalletBatch& batch, const CExtKey& master_key, OutputType addr_type, bool internal)
{
    ...
    if (addr_type == OutputType::PQ) {
        // Blazecoin P2PQH (PQ_SIGNATURES.md s6): seeded from the same HD master key, stored as its own
        // 32-byte secret so the existing (plaintext / encrypted) descriptor-key path carries it.
        CKey seed_key;
        m_wallet_descriptor = GeneratePQWalletDescriptor(master_key.key, internal, seed_key);
        if (!AddDescriptorKeyWithDB(batch, seed_key, seed_key.GetPubKey())) {
            throw std::runtime_error(std::string(__func__) + ": writing post-quantum descriptor master seed failed");
        }
    } else {
...
SigningResult DescriptorScriptPubKeyMan::SignMessagePQ(const std::string& message, const PQKeyHash& pqkh, std::string& str_sig) const
{
    std::unique_ptr<FlatSigningProvider> keys = GetSigningProvider(GetScriptForDestination(pqkh), true);
    if (!keys) {
        return SigningResult::PRIVATE_KEY_NOT_AVAILABLE;
    }

    PQKeyPair key;
    if (!keys->GetPQKey(uint256(pqkh), key) || key.seckey.empty()) {
        return SigningResult::PRIVATE_KEY_NOT_AVAILABLE;
    }

    if (!MessageSignPQ(key.pubkey, key.seckey, message, str_sig)) {
        return SigningResult::SIGNING_FAILED;
    }
    return SigningResult::OK;
}

...
SigningResult CWallet::SignMessagePQ(const std::string& message, const PQKeyHash& pqkh, std::string& str_sig) const
{
    SignatureData sigdata;
    CScript script_pub_key = GetScriptForDestination(pqkh);
    for (const auto& spk_man_pair : m_spk_managers) {
        if (spk_man_pair.second->CanProvide(script_pub_key, sigdata)) {
            LOCK(cs_wallet);  // DescriptorScriptPubKeyMan calls IsLocked which can lock cs_wallet in a deadlocking order
            return spk_man_pair.second->SignMessagePQ(message, pqkh, str_sig);
        }
    }
    return SigningResult::PRIVATE_KEY_NOT_AVAILABLE;
}

int CWallet::PQSigActivationHeight() const
{
    return Params().GetConsensus().DeploymentHeight(Consensus::DEPLOYMENT_PQSIG);
}

bool CWallet::IsPQSigActiveForNextBlock() const
{
    AssertLockHeld(cs_wallet);
    // Same rule as DeploymentActiveAfter(tip): the NEXT block's height is tip + 1. A wallet that has
    // not processed any block yet (m_last_block_processed_height == -1) is at height "before genesis".
    return m_last_block_processed_height + 1 >= PQSigActivationHeight();
}
...
util::Result<CTxDestination> CWallet::GetNewDestination(const OutputType type, const std::string label)
{
    ...
    if (!spk_man) {
        if (type == OutputType::PQ) {
            // Blazecoin: a wallet created before P2PQH support has no post-quantum key manager (it is
            // set up at wallet creation from the HD master key); there is no in-place upgrade path yet.
            return util::Error{_("Error: This wallet has no post-quantum (pq) key manager - it was created before P2PQH support. Create a new descriptor wallet to receive at BQ addresses.")};
        }

...
static const std::string OUTPUT_TYPE_STRING_PQ = "pq"; // Blazecoin P2PQH
...
CTxDestination GetDestinationForKey(const CPubKey& key, OutputType type)
{
    ...
    case OutputType::PQ: // Blazecoin: P2PQH keys are ML-DSA, never a CPubKey (descriptor wallets only)
    case OutputType::UNKNOWN: {} // This function should never be used with BECH32M, PQ or UNKNOWN, so let it assert
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
CTxDestination AddAndGetDestinationForScript(FlatSigningProvider& keystore, const CScript& script, OutputType type)
{
    ...
    case OutputType::BECH32M:
    case OutputType::PQ: // Blazecoin: a P2PQH output is never a wrapped script
    case OutputType::UNKNOWN: {} // This function should not be used for BECH32M, PQ or UNKNOWN, so let it assert
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
std::optional<OutputType> OutputTypeFromDestination(const CTxDestination& dest) {
    ...
    if (std::holds_alternative<PQKeyHash>(dest)) {
        return OutputType::PQ;
    }
    return std::nullopt;
}

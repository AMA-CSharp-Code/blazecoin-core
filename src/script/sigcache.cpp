...
void SignatureCache::ComputeEntryPQ(uint256& entry, const uint256& hash, Span<const unsigned char> sig, Span<const unsigned char> pubkey) const
{
    CSHA256 hasher = m_salted_hasher_pq;
    hasher.Write(hash.begin(), 32).Write(pubkey.data(), pubkey.size()).Write(sig.data(), sig.size()).Finalize(entry.begin());
}

bool CachingTransactionSignatureChecker::VerifyPQSignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, const uint256& sighash) const
{
    uint256 entry;
    m_signature_cache.ComputeEntryPQ(entry, sighash, sig, pubkey);
    if (m_signature_cache.Get(entry, !store)) return true;
    if (!TransactionSignatureChecker::VerifyPQSignature(sig, pubkey, sighash)) return false;

...
{
private:
    //! Entries are SHA256(nonce || 'E' or 'S' or 'P' || 31 zero bytes || signature hash || public key || signature):
    CSHA256 m_salted_hasher_ecdsa;
    CSHA256 m_salted_hasher_schnorr;
    CSHA256 m_salted_hasher_pq; // Blazecoin ML-DSA-44 (OP_CHECKPQSIG); the key blob stands in for the public key
...
public:
    ...
    /** Blazecoin: ML-DSA-44 verify is ~130 us, so P2PQH spends are cached like every other signature. */
    void ComputeEntryPQ(uint256& entry, const uint256 &hash, Span<const unsigned char> sig, Span<const unsigned char> pubkey) const;
...
public:
    ...
    bool VerifyPQSignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, const uint256& sighash) const override;
};

...
private:
    ...
    /** Blazecoin: map derivation index -> expanded ML-DSA-44 public key of a ranged pqkh(mldsa44:.../*)
     *  descriptor (PQ_SIGNATURES.md s6). ML-DSA has no public derivation, so this is the only way to
     *  expand the descriptor without the master seed (locked wallet, wallet load). */
...
public:
    ...
    /** Blazecoin: cache the ML-DSA-44 public key expanded at der_index of a pqkh() descriptor */
    void CachePQPubKey(uint32_t der_index, const std::vector<unsigned char>& pubkey);
    /** Blazecoin: retrieve the cached ML-DSA-44 public key at der_index */
    bool GetCachedPQPubKey(uint32_t der_index, std::vector<unsigned char>& pubkey) const;
    /** Blazecoin: retrieve all cached ML-DSA-44 public keys */
    const std::unordered_map<uint32_t, std::vector<unsigned char>>& GetCachedPQPubKeys() const { return m_pq_pubkeys; }

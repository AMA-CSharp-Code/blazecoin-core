...
std::string GetTxnOutputType(TxoutType t)
{
    ...
    case TxoutType::PQ_KEYHASH: return "pqkh"; // Blazecoin P2PQH (PQ_SIGNATURES.md s5: validateaddress/decodescript "type": "pqkh")
...
TxoutType Solver(const CScript& scriptPubKey, std::vector<std::vector<unsigned char>>& vSolutionsRet)
{
    ...
    // Blazecoin P2PQH: exact 34-byte template 0x20 <pqkh> OP_CHECKPQSIG (PQ_SIGNATURES.md s3.2).
    {
        uint256 pqkh;
        if (pq::IsPayToPQKeyHash(scriptPubKey, &pqkh)) {
            vSolutionsRet.push_back(std::vector<unsigned char>(pqkh.begin(), pqkh.end()));
            return TxoutType::PQ_KEYHASH;
        }
    }

...
static bool EvalChecksig(const valtype& sig, const valtype& pubkey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, ScriptExecutionData& execdata, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, bool& success)
{
    ...
    case SigVersion::PQ:
        // Blazecoin: the PQ context defines OP_CHECKPQSIG only; ECDSA checksigs are bad opcodes there.
...
/** Blazecoin OP_CHECKPQSIG (PQ_SIGNATURES.md s3.4 steps 2-7). Stack: <sig> <keyblob> <pqkh>
 *  (pqkh on top, pushed by the P2PQH scriptPubKey). Any failure is a script failure with a
 *  distinct SCRIPT_ERR_PQ_* code; on success the three elements are replaced by true. */
static bool EvalCheckPQSig(std::vector<valtype>& stack, const CScript& scriptCode, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror)
{
    assert(sigversion == SigVersion::PQ);
    if (stack.size() < 3) return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);

    const valtype& pqkh = stacktop(-1);
    const valtype& keyblob = stacktop(-2);
    const valtype& sig = stacktop(-3);

    // Step 3: the blob must hash to the committed key hash (algo id is inside the hash).
    if (pqkh.size() != pq::PQ_KEYHASH_SIZE) return set_error(serror, SCRIPT_ERR_PQ_KEYHASH_MISMATCH);
    if (pq::PQKeyHash(keyblob) != uint256(pqkh)) return set_error(serror, SCRIPT_ERR_PQ_KEYHASH_MISMATCH);
    // Step 4: only an ACTIVE algorithm id, with exactly the right pubkey length.
    if (keyblob.empty()) return set_error(serror, SCRIPT_ERR_PQ_PUBKEY_LENGTH);
    if (keyblob[0] != pq::PQ_ALGO_MLDSA44) return set_error(serror, SCRIPT_ERR_PQ_ALGO_INACTIVE);
    if (keyblob.size() != pq::PQ_MLDSA44_KEYBLOB_SIZE) return set_error(serror, SCRIPT_ERR_PQ_PUBKEY_LENGTH);
    if (sig.size() != pq::PQ_MLDSA44_SIG_SIZE) return set_error(serror, SCRIPT_ERR_PQ_SIG_INVALID);
    // Steps 5-6: PQSigHash + ML-DSA-44 verify under ctx "blazecoin-tx-v1".
    if (!checker.CheckPQSignature(sig, keyblob, scriptCode, sigversion)) return set_error(serror, SCRIPT_ERR_PQ_SIG_INVALID);
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    assert(sigversion == SigVersion::BASE || sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::TAPSCRIPT || sigversion == SigVersion::PQ);
    ...
    // Blazecoin PQ context (PQ_SIGNATURES.md s3.4): 5,000-byte elements instead of 520.
    const unsigned int max_element_size = (sigversion == SigVersion::PQ) ? pq::MAX_PQ_SCRIPT_ELEMENT_SIZE : MAX_SCRIPT_ELEMENT_SIZE;
    int nOpCount = 0;
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            if (sigversion == SigVersion::PQ && opcode > OP_16 && opcode != OP_CHECKPQSIG) {
                // The PQ context is deliberately minimal: pushes and OP_CHECKPQSIG only.
                // (The scriptSig is push-only by rule and the scriptPubKey is the fixed template.)
                return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
            }
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGADD: // == OP_CHECKPQSIG (0xba): meaning selected by SigVersion
                {
                    // Blazecoin: in the PQ context this byte is OP_CHECKPQSIG (PQ_SIGNATURES.md s3.4).
                    if (sigversion == SigVersion::PQ) {
                        if (!EvalCheckPQSig(stack, script, checker, sigversion, serror)) return false;
                        break;
                    }
...
void PrecomputedTransactionData::Init(const T& txTo, std::vector<CTxOut>&& spent_outputs, bool force)
{
    ...
    for (size_t inpos = 0; inpos < txTo.vin.size() && !(uses_bip143_segwit && uses_bip341_taproot); ++inpos) {
        ...
        // Blazecoin: a P2PQH spend reuses the BIP-143 hashPrevouts/hashSequence/hashOutputs (PQ_SIGNATURES.md s3.5).
        if (m_spent_outputs_ready && pq::IsPayToPQKeyHash(m_spent_outputs[inpos].scriptPubKey)) {
            uses_bip143_segwit = true;
        }
...
template <class T>
bool GenericTransactionSignatureChecker<T>::VerifyPQSignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, const uint256& sighash) const
{
    const Span<const unsigned char> ctx{reinterpret_cast<const unsigned char*>(pq::CTX_TX.data()), pq::CTX_TX.size()};
    return mldsa44::Verify(pubkey, sighash, ctx, sig);
}

template <class T>
bool GenericTransactionSignatureChecker<T>::CheckPQSignature(Span<const unsigned char> sig, Span<const unsigned char> keyblob, const CScript& scriptCode, SigVersion sigversion) const
{
    assert(sigversion == SigVersion::PQ);
    // The interpreter has already enforced the blob shape; be defensive anyway.
    if (keyblob.size() != pq::PQ_MLDSA44_KEYBLOB_SIZE || keyblob[0] != pq::PQ_ALGO_MLDSA44) return false;
    if (sig.size() != pq::PQ_MLDSA44_SIG_SIZE) return false;
    // The digest commits to the amount (PQ_SIGNATURES.md s3.5), so it is required.
    if (amount < 0) return HandleMissingData(m_mdb);
    const uint256 sighash = pq::PQSigHash(*txTo, nIn, amount, scriptCode, this->txdata);
    return VerifyPQSignature(sig, keyblob.subspan(1), sighash);
}
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    // Blazecoin P2PQH (PQ_SIGNATURES.md s3.4): once DEPLOYMENT_PQSIG is active the exact
    // 34-byte template is evaluated in SigVersion::PQ. Without the flag it falls through to
    // BASE evaluation below and fails on push size / bad opcode - the intended fail-closed
    // behaviour before H_Q (s3.6); it is never a NOP-style anyone-can-spend.
    if ((flags & SCRIPT_VERIFY_PQSIG) && pq::IsPayToPQKeyHash(scriptPubKey)) {
        if (!scriptSig.IsPushOnly()) return set_error(serror, SCRIPT_ERR_SIG_PUSHONLY);
        if ((flags & SCRIPT_VERIFY_WITNESS) && !witness->IsNull()) return set_error(serror, SCRIPT_ERR_WITNESS_UNEXPECTED);
        std::vector<std::vector<unsigned char> > pqstack;
        if (!EvalScript(pqstack, scriptSig, flags, checker, SigVersion::PQ, serror)) return false; // serror is set
        if (pqstack.size() != 2) return set_error(serror, SCRIPT_ERR_PQ_STACK_SIZE);
        if (!EvalScript(pqstack, scriptPubKey, flags, checker, SigVersion::PQ, serror)) return false; // serror is set
        if (pqstack.size() != 1 || !CastToBool(pqstack.back())) return set_error(serror, SCRIPT_ERR_EVAL_FALSE);
        return set_success(serror);
    }

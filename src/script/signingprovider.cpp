...
bool HidingSigningProvider::GetPQKey(const uint256& pqkh, PQKeyPair& key) const
{
    if (!m_provider->GetPQKey(pqkh, key)) return false;
    if (m_hide_secret) key.seckey.clear();
    return true;
}
...
bool FlatSigningProvider::GetPQKey(const uint256& pqkh, PQKeyPair& key) const
{
    return LookupHelper(pq_keys, pqkh, key);
}
...
FlatSigningProvider& FlatSigningProvider::Merge(FlatSigningProvider&& b)
{
    ...
    // Blazecoin: a private entry (seckey present) must win over a public-only one for the same pqkh.
    for (auto& [pqkh, key] : b.pq_keys) {
        auto it = pq_keys.find(pqkh);
        if (it == pq_keys.end() || (it->second.seckey.empty() && !key.seckey.empty())) {
            pq_keys[pqkh] = std::move(key);
        }
    }
    b.pq_keys.clear();
    return *this;
...
bool MultiSigningProvider::GetPQKey(const uint256& pqkh, PQKeyPair& key) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetPQKey(pqkh, key)) return true;
    }
    return false;
}

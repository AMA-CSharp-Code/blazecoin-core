...
/** Blazecoin P2PQH key material (PQ_SIGNATURES.md s6): the ML-DSA-44 public key
 *  and, when available, the 2,560-byte secret key, indexed by pqkh. The secret
 *  key is empty for a public-only (watch / solving) provider. */
struct PQKeyPair
{
...
public:
    ...
    /** Blazecoin: ML-DSA-44 key for the P2PQH commitment pqkh (seckey empty when only the public key is known). */
    virtual bool GetPQKey(const uint256& pqkh, PQKeyPair& key) const { return false; }
...
public:
    ...
    bool GetPQKey(const uint256& pqkh, PQKeyPair& key) const override;
};
...
struct FlatSigningProvider final : public SigningProvider
{
    ...
    std::map<uint256, PQKeyPair> pq_keys; /** Blazecoin: ML-DSA-44 keys by pqkh (PQ_SIGNATURES.md s6); seckey empty in public-only providers */

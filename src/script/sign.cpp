...
/** Blazecoin: the flags a signer verifies its own work against. SCRIPT_VERIFY_PQSIG is a hard-fork
 *  flag that only selects the P2PQH evaluation path for the exact 34-byte template; whether a P2PQH
 *  spend may be broadcast yet is a policy question answered by the wallet, not here. */
static constexpr unsigned int SIGNING_SCRIPT_VERIFY_FLAGS{STANDARD_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_PQSIG};
...
bool MutableTransactionSignatureCreator::CreatePQSig(const SigningProvider& provider, std::vector<unsigned char>& sig, std::vector<unsigned char>& keyblob, const uint256& pqkh, const CScript& scriptCode) const
{
    // v1 has exactly one sighash mode, SIGHASH_ALL, implicit in the digest (PQ_SIGNATURES.md s3.5).
    if (nHashType != SIGHASH_DEFAULT && nHashType != SIGHASH_ALL) return false;
    PQKeyPair key;
    if (!provider.GetPQKey(pqkh, key)) return false;
    if (key.pubkey.size() != mldsa44::PUBKEY_SIZE || key.seckey.size() != mldsa44::SECKEY_SIZE) return false;
    keyblob = pq::PQKeyBlob(key.pubkey);
    if (pq::PQKeyHash(keyblob) != pqkh) return false;
    // The digest commits to the amount, so signing without it is impossible.
    if (!MoneyRange(amount)) return false;
    const uint256 hash = pq::PQSigHash(m_txto, nIn, amount, scriptCode, m_txdata);
    const Span<const unsigned char> ctx{reinterpret_cast<const unsigned char*>(pq::CTX_TX.data()), pq::CTX_TX.size()};
    return mldsa44::Sign(key.seckey, hash, ctx, sig) && sig.size() == mldsa44::SIG_SIZE;
}
                     ...
                     std::vector<valtype>& ret, TxoutType& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    case TxoutType::PQ_KEYHASH: {
        // Blazecoin P2PQH (PQ_SIGNATURES.md s3.3): scriptSig = <sig:2420> <keyblob:1313>, top level only.
        if (sigversion != SigVersion::BASE) return false;
        const uint256 pqkh{vSolutions[0]};
        std::vector<unsigned char> keyblob;
        if (sigdata.pq_sig.size() == pq::PQ_MLDSA44_SIG_SIZE && sigdata.pq_keyblob.size() == pq::PQ_MLDSA44_KEYBLOB_SIZE &&
            pq::PQKeyHash(sigdata.pq_keyblob) == pqkh) {
            // A signature already supplied (PSBT proprietary fields) - assemble without a key.
            sig = sigdata.pq_sig;
            keyblob = sigdata.pq_keyblob;
        } else if (!creator.CreatePQSig(provider, sig, keyblob, pqkh, scriptPubKey)) {
            return false;
        }
        sigdata.pq_sig = sig;
        sigdata.pq_keyblob = keyblob;
        ret.push_back(std::move(sig));
        ret.push_back(std::move(keyblob));
        return true;
    }
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    if (solved && whichType == TxoutType::SCRIPTHASH)
    {
        ...
        solved = solved && SignStep(provider, creator, subscript, result, whichType, SigVersion::BASE, sigdata) && whichType != TxoutType::SCRIPTHASH && whichType != TxoutType::PQ_KEYHASH;
        P2SH = true;
    }
    ...
    // Test solution
    sigdata.complete = solved && VerifyScript(sigdata.scriptSig, fromPubKey, &sigdata.scriptWitness, SIGNING_SCRIPT_VERIFY_FLAGS, creator.Checker());
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const CTxOut& txout)
{
    ...
    if (VerifyScript(data.scriptSig, txout.scriptPubKey, &data.scriptWitness, SIGNING_SCRIPT_VERIFY_FLAGS, extractor_checker)) {
        data.complete = true;
        return data;
    }
...
void SignatureData::MergeSignatureData(SignatureData sigdata)
{
    ...
    if (pq_sig.empty() && !sigdata.pq_sig.empty()) {
        pq_sig = std::move(sigdata.pq_sig);
        pq_keyblob = std::move(sigdata.pq_keyblob);
    }
}
...
public:
    ...
    bool CheckPQSignature(Span<const unsigned char> sig, Span<const unsigned char> keyblob, const CScript& scriptCode, SigVersion sigversion) const override { return sig.size() != 0; }
...
public:
    ...
    bool CreatePQSig(const SigningProvider& provider, std::vector<unsigned char>& sig, std::vector<unsigned char>& keyblob, const uint256& pqkh, const CScript& scriptCode) const override
    {
        // Blazecoin: a 2,420-byte dummy ML-DSA-44 signature; the real key blob when the provider knows
        // the public key (so the pqkh check passes under the dummy checker), an all-zero blob otherwise.
        sig.assign(pq::PQ_MLDSA44_SIG_SIZE, '\000');
        PQKeyPair key;
        if (provider.GetPQKey(pqkh, key) && key.pubkey.size() == pq::PQ_MLDSA44_PUBKEY_SIZE) {
            keyblob = pq::PQKeyBlob(key.pubkey);
        } else {
            keyblob = pq::PQKeyBlob(std::vector<unsigned char>(pq::PQ_MLDSA44_PUBKEY_SIZE, 0));
        }
        return true;
    }
};
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        if (!sigdata.complete && !VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, SIGNING_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, amount, txdata, MissingDataBehavior::FAIL), &serror)) {

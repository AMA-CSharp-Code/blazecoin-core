...
std::string ScriptErrorString(const ScriptError serror)
{
    switch (serror)
    {
        ...
        case SCRIPT_ERR_PQ_STACK_SIZE:
            return "P2PQH scriptSig must leave exactly two stack elements";
        case SCRIPT_ERR_PQ_KEYHASH_MISMATCH:
            return "PQ key blob does not hash to the committed key hash";
        case SCRIPT_ERR_PQ_ALGO_INACTIVE:
            return "PQ key blob algorithm id is not active";
        case SCRIPT_ERR_PQ_PUBKEY_LENGTH:
            return "PQ key blob has the wrong public key length";
        case SCRIPT_ERR_PQ_SIG_INVALID:
            return "PQ signature is invalid";

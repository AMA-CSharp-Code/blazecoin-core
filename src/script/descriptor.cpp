...
/** Blazecoin: a parsed pqkh(KEY) descriptor - P2PQH, scriptPubKey 0x20 <pqkh> OP_CHECKPQSIG
 *  (PQ_SIGNATURES.md s3, s6). The key is ML-DSA-44, never a secp256k1 key, so this descriptor
 *  bypasses PubkeyProvider entirely. Two key forms:
 *
 *   - ranged seed form:  pqkh(mldsa44:<32-byte master seed, hex>/*)   private
 *                        pqkh(mldsa44:<20-byte seed id, hex>/*)       public / normalized / compat
 *     key i:  xi_i = pq::PQSeedForIndex(master_seed, i);  (pk_i, sk_i) = ML-DSA-44.KeyGen_internal(xi_i)
 *     ML-DSA has no public derivation: the public form can only be expanded from a DescriptorCache
 *     holding the already-expanded public keys (locked or loading wallet) or with the seed itself.
 *     The seed rides the wallet's existing descriptor-key machinery as a CKey: the 32-byte seed IS
 *     the secret, the compressed secp256k1 public key of that secret is the "marker" pubkey the key
 *     record is filed under (and encrypted against), and seed id = Hash160(marker pubkey), i.e. the
 *     CKeyID under which SigningProvider::GetKey returns the seed.
 *   - single public form: pqkh(mldsa44pub:<1312-byte ML-DSA-44 public key, hex>)
 *     what InferDescriptor returns for a P2PQH scriptPubKey whose key the provider knows; importable
 *     as a watch-only single address.
 */
class PQKHDescriptor final : public DescriptorImpl
{
    const std::optional<CKeyID> m_seed_id;      //!< ranged seed form: seed id (CKeyID of the seed's marker pubkey)
    const std::vector<unsigned char> m_pubkey;  //!< single public form: the ML-DSA-44 public key

    static bool SeedFromProvider(const SigningProvider& arg, const CKeyID& seed_id, std::vector<unsigned char>& seed32)
    {
        CKey key;
        if (!arg.GetKey(seed_id, key) || !key.IsValid()) return false;
        seed32.assign(UCharCast(key.begin()), UCharCast(key.end()));
        return seed32.size() == mldsa44::SEED_SIZE;
    }

    static bool DeriveAt(const std::vector<unsigned char>& seed32, uint32_t index, PQKeyPair& out)
    {
        const uint256 xi = pq::PQSeedForIndex(seed32, index);
        return mldsa44::KeyGenFromSeed(xi, out.pubkey, out.seckey);
    }

    static CScript ScriptFor(const std::vector<unsigned char>& pubkey, uint256& pqkh)
    {
        pqkh = pq::PQKeyHash(pq::PQKeyBlob(pubkey));
        return pq::GetScriptForPQKeyHash(pqkh);
    }

protected:
    std::vector<CScript> MakeScripts(const std::vector<CPubKey>&, Span<const CScript>, FlatSigningProvider&) const override
    {
        assert(false); // ExpandHelper is overridden; the CPubKey path is never taken
        return {};
    }
    ...
    {
        if (!m_seed_id) {
            out = "pqkh(mldsa44pub:" + HexStr(m_pubkey) + ")";
            return true;
        }
        if (type == StringType::PRIVATE) {
            std::vector<unsigned char> seed32;
            if (!arg || !SeedFromProvider(*arg, *m_seed_id, seed32)) return false;
            out = "pqkh(mldsa44:" + HexStr(seed32) + "/*)";
            return true;
        }
        out = "pqkh(mldsa44:" + HexStr(*m_seed_id) + "/*)";
        return true;
    }

public:
    /** Ranged seed form. */
    explicit PQKHDescriptor(const CKeyID& seed_id) : DescriptorImpl({}, "pqkh"), m_seed_id(seed_id) {}
    /** Single public form. */
    explicit PQKHDescriptor(std::vector<unsigned char> pubkey) : DescriptorImpl({}, "pqkh"), m_pubkey(std::move(pubkey)) {}

    bool IsRange() const override { return m_seed_id.has_value(); }
    ...
    bool ExpandHelper(int pos, const SigningProvider& arg, const DescriptorCache* read_cache, std::vector<CScript>& output_scripts, FlatSigningProvider& out, DescriptorCache* write_cache) const override
    {
        ...
        if (!m_seed_id) {
            ...
            const uint32_t index = static_cast<uint32_t>(pos);
            if (!read_cache || !read_cache->GetCachedPQPubKey(index, pubkey)) {
                std::vector<unsigned char> seed32;
                PQKeyPair derived;
                if (!SeedFromProvider(arg, *m_seed_id, seed32) || !DeriveAt(seed32, index, derived)) return false;
                pubkey = std::move(derived.pubkey);
            }
            if (write_cache) write_cache->CachePQPubKey(index, pubkey);
        }
        uint256 pqkh;
        const CScript script = ScriptFor(pubkey, pqkh);
        auto& entry = out.pq_keys[pqkh];
        if (entry.pubkey.empty()) entry.pubkey = pubkey; // never clobber a private entry already present
        output_scripts = Vector(script);
        return true;
    }
    ...
    void ExpandPrivate(int pos, const SigningProvider& provider, FlatSigningProvider& out) const override
    {
        ...
        std::vector<unsigned char> seed32;
        PQKeyPair derived;
        if (!SeedFromProvider(provider, *m_seed_id, seed32) || !DeriveAt(seed32, static_cast<uint32_t>(pos), derived)) return;
        uint256 pqkh;
        ScriptFor(derived.pubkey, pqkh);
        out.pq_keys[pqkh] = std::move(derived);
        // The seed itself, so callers that check "are all private keys present" via out.keys see it.
        CKey seed_key;
        if (provider.GetKey(*m_seed_id, seed_key)) out.keys.emplace(*m_seed_id, seed_key);
    }

    std::optional<int64_t> ScriptSize() const override { return pq::PQ_SCRIPTPUBKEY_SIZE; }

    /** <sig:2420> <keyblob:1313> with their 3-byte pushes = 3,739 bytes (PQ_SIGNATURES.md s3.3, s4). */
    std::optional<int64_t> MaxSatSize(bool) const override
    {
        return 3 + static_cast<int64_t>(pq::PQ_MLDSA44_SIG_SIZE) + 3 + static_cast<int64_t>(pq::PQ_MLDSA44_KEYBLOB_SIZE);
    }
    std::optional<int64_t> MaxSatisfactionWeight(bool use_max_sig) const override { return *MaxSatSize(use_max_sig) * WITNESS_SCALE_FACTOR; }
    std::optional<int64_t> MaxSatisfactionElems() const override { return 2; }
};
...
std::unique_ptr<DescriptorImpl> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    // Blazecoin P2PQH (PQ_SIGNATURES.md s6): pqkh(mldsa44:<seed>/*), pqkh(mldsa44:<seed id>/*) or pqkh(mldsa44pub:<pubkey>).
    if (Func("pqkh", expr)) {
        if (ctx != ParseScriptContext::TOP) {
            error = "Can only have pqkh() at top level";
            return nullptr;
        }
        const std::string arg(expr.begin(), expr.end());
        static const std::string PUB_PREFIX{"mldsa44pub:"};
        static const std::string SEED_PREFIX{"mldsa44:"};
        if (arg.rfind(PUB_PREFIX, 0) == 0) {
            const std::string hex = arg.substr(PUB_PREFIX.size());
            if (!IsHex(hex) || hex.size() != 2 * pq::PQ_MLDSA44_PUBKEY_SIZE) {
                error = strprintf("pqkh(): mldsa44pub key must be %u hex bytes", pq::PQ_MLDSA44_PUBKEY_SIZE);
                return nullptr;
            }
            return std::make_unique<PQKHDescriptor>(ParseHex(hex));
        }
        if (arg.rfind(SEED_PREFIX, 0) == 0) {
            std::string hex = arg.substr(SEED_PREFIX.size());
            if (hex.size() < 2 || hex.compare(hex.size() - 2, 2, "/*") != 0) {
                error = "pqkh(): mldsa44 seed form must be ranged (end with /*)";
                return nullptr;
            }
            hex.resize(hex.size() - 2);
            if (!IsHex(hex)) {
                error = "pqkh(): seed is not hex";
                return nullptr;
            }
            const std::vector<unsigned char> bytes = ParseHex(hex);
            if (bytes.size() == mldsa44::SEED_SIZE) {
                CKey seed_key;
                seed_key.Set(bytes.begin(), bytes.end(), true);
                if (!seed_key.IsValid()) {
                    error = "pqkh(): master seed is not usable as key material";
                    return nullptr;
                }
                const CKeyID seed_id = seed_key.GetPubKey().GetID();
                out.keys.emplace(seed_id, seed_key);
                return std::make_unique<PQKHDescriptor>(seed_id);
            }
            if (bytes.size() == 20) {
                return std::make_unique<PQKHDescriptor>(CKeyID(uint160(bytes)));
            }
            error = "pqkh(): mldsa44 seed must be 32 bytes (private) or a 20-byte seed id (public)";
            return nullptr;
        }
        error = "pqkh(): key must be mldsa44:<seed>/* or mldsa44pub:<pubkey>";
        return nullptr;
    }
...
std::unique_ptr<DescriptorImpl> InferScript(const CScript& script, ParseScriptContext ctx, const SigningProvider& provider)
{
    ...
    // Blazecoin P2PQH: solvable only when the provider knows the ML-DSA-44 key behind the pqkh.
    if (txntype == TxoutType::PQ_KEYHASH && ctx == ParseScriptContext::TOP) {
        PQKeyPair key;
        if (provider.GetPQKey(uint256(data[0]), key) && key.pubkey.size() == pq::PQ_MLDSA44_PUBKEY_SIZE) {
            return std::make_unique<PQKHDescriptor>(key.pubkey);
        }
    }
...
DescriptorCache DescriptorCache::MergeAndDiff(const DescriptorCache& other)
{
    ...
    for (const auto& [der_index, pubkey] : other.GetCachedPQPubKeys()) {
        ...
        if (GetCachedPQPubKey(der_index, existing)) {
            if (existing != pubkey) {
                throw std::runtime_error(std::string(__func__) + ": New cached PQ pubkey does not match already cached PQ pubkey");
            }
            continue;
        }

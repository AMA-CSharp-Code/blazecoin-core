...
public:
    ...
    /** Blazecoin P2PQH (PQ_SIGNATURES.md s3.3-s3.5): ML-DSA-44 signature over PQSigHash for the key
     *  committed to by pqkh, plus the key blob (algo_id || pubkey) the scriptSig must push after it. */
    virtual bool CreatePQSig(const SigningProvider& provider, std::vector<unsigned char>& sig, std::vector<unsigned char>& keyblob, const uint256& pqkh, const CScript& scriptCode) const =0;
};
...
struct SignatureData {
    ...
    std::vector<unsigned char> pq_sig; ///< Blazecoin P2PQH: ML-DSA-44 signature (2,420 bytes) for this input, carried in PSBT proprietary field "blz"/0x01
    std::vector<unsigned char> pq_keyblob; ///< Blazecoin P2PQH: key blob algo_id||pubkey (1,313 bytes), PSBT proprietary field "blz"/0x02

...
enum : uint32_t {
    ...
    // Blazecoin: P2PQH / OP_CHECKPQSIG active (PQ_SIGNATURES.md s3.4, s7).
    // Set by GetBlockScriptFlags from DEPLOYMENT_PQSIG. Unlike every flag above
    // this is a HARD fork: it makes the 34-byte P2PQH template spendable, which
    // it is not (bad opcode / push size) without the flag. Bit 21 because
    // bits 19 and 20 are taken upstream (the spec's "1U << 19" collides with
    // SCRIPT_VERIFY_DISCOURAGE_OP_SUCCESS).
    SCRIPT_VERIFY_PQSIG = (1U << 21),
...
enum class SigVersion
{
    ...
    PQ = 4,          //!< Blazecoin P2PQH spend (prevout is the 34-byte template, SCRIPT_VERIFY_PQSIG set); OP_CHECKPQSIG defined, 5000-byte elements; see PQ_SIGNATURES.md s3.4
};
...
public:
    ...
    /** Blazecoin OP_CHECKPQSIG (PQ_SIGNATURES.md s3.4 steps 5-6): keyblob is
     *  the full algo_id||pubkey blob (already length/algo-checked by the
     *  interpreter), scriptCode the 34-byte P2PQH scriptPubKey being spent. */
    virtual bool CheckPQSignature(Span<const unsigned char> sig, Span<const unsigned char> keyblob, const CScript& scriptCode, SigVersion sigversion) const
    {
        return false;
    }
...
protected:
    ...
    /** ML-DSA-44 verify of sighash under ctx "blazecoin-tx-v1"; pubkey is the bare 1312-byte key. Overridden by the caching checker. */
    virtual bool VerifyPQSignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, const uint256& sighash) const;
...
public:
    ...
    bool CheckPQSignature(Span<const unsigned char> sig, Span<const unsigned char> keyblob, const CScript& scriptCode, SigVersion sigversion) const override;
...
public:
    ...
    bool CheckPQSignature(Span<const unsigned char> sig, Span<const unsigned char> keyblob, const CScript& scriptCode, SigVersion sigversion) const override
    {
        return m_checker.CheckPQSignature(sig, keyblob, scriptCode, sigversion);
    }

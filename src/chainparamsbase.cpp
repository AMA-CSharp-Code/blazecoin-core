...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-testactivationheight=name@height.", "Set the activation height of 'name' (segwit, bip34, dersig, cltv, csv, phoenix413, pqsig). (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const ChainType chain)
{
    switch (chain) {
    case ChainType::MAIN:
        return std::make_unique<CBaseChainParams>("", 55413, 55415);
    case ChainType::TESTNET:
        // Legacy Blazecoin testnet RPC port was 75413 (overflows uint16_t in modern
        // Core); 55423/55425 mirrors mainnet's 55413/55415 by convention.
        return std::make_unique<CBaseChainParams>("testnet3", 55423, 55425);

...
public:
    ...
    std::string operator()(const PQKeyHash& id) const
    {
        // Blazecoin P2PQH (PQ_SIGNATURES.md s5): Base58Check(PQ_ADDRESS prefix || pqkh) -> "BQ..." / "TQ...", 52 chars
        std::vector<unsigned char> data = m_params.Base58Prefix(CChainParams::PQ_ADDRESS);
        data.insert(data.end(), id.begin(), id.end());
        return EncodeBase58Check(data);
    }
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    // Blazecoin: the cap is 34 (2-byte PQ_ADDRESS prefix + 32-byte pqkh), not 21, so "BQ..." addresses decode.
    if (!is_bech32 && DecodeBase58Check(str, data, 34)) {
        ...
        // Blazecoin P2PQH addresses (PQ_SIGNATURES.md s5): two-byte prefix + the 32-byte pqkh.
        const std::vector<unsigned char>& pq_prefix = params.Base58Prefix(CChainParams::PQ_ADDRESS);
        if (data.size() == uint256::size() + pq_prefix.size() && std::equal(pq_prefix.begin(), pq_prefix.end(), data.begin())) {
            PQKeyHash pqkh;
            std::copy(data.begin() + pq_prefix.size(), data.end(), pqkh.begin());
            return pqkh;
        }
        ...
        if ((data.size() >= script_prefix.size() &&
            ...
            (data.size() >= pq_prefix.size() &&
                std::equal(pq_prefix.begin(), pq_prefix.end(), data.begin()))) {
            error_str = "Invalid length for Base58 address (P2PKH, P2SH or P2PQH)";
        } else {

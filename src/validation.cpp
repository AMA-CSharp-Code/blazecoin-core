    ...
    bool fAddToMempool)
{
    ...
    const auto filter_final_and_mature = [&](CTxMemPool::txiter it)
        ...
        // Blazecoin: a reorg back across H_Q makes every P2PQH spend invalid for the next block
        // (SCRIPT_VERIFY_PQSIG is a hard-fork flag; PQ_SIGNATURES.md s7). Drop them here so the
        // miner never builds a block from them.
        if (!DeploymentActiveAfter(m_chain.Tip(), m_chainman, Consensus::DEPLOYMENT_PQSIG)) {
            const CCoinsViewMemPool view_mempool{&CoinsTip(), *m_mempool};
            for (const CTxIn& txin : tx.vin) {
                Coin coin;
                if (view_mempool.GetCoin(txin.prevout, coin) && pq::IsPayToPQKeyHash(coin.out.scriptPubKey)) {
                    return true;
                }
            }
        }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Blazecoin: the mempool holds transactions for the NEXT block, so P2PQH policy
    // (outputs standard, 3,739-byte scriptSigs, PQ inputs) follows the next block's
    // DEPLOYMENT_PQSIG state (PQ_SIGNATURES.md s3.6).
    const bool pqsig_active{DeploymentActiveAfter(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, Consensus::DEPLOYMENT_PQSIG)};

    // Rather not work on nonstandard transactions (unless -testnet/-regtest)
    std::string reason;
    if (m_pool.m_opts.require_standard && !IsStandardTx(tx, m_pool.m_opts.max_datacarrier_bytes, m_pool.m_opts.permit_bare_multisig, m_pool.m_opts.dust_relay_feerate, reason, pqsig_active)) {
    ...
    if (m_pool.m_opts.require_standard && !AreInputsStandard(tx, m_view, pqsig_active)) {
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // Blazecoin: P2PQH spends validate for the NEXT block (PQ_SIGNATURES.md s7).
    if (DeploymentActiveAfter(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, Consensus::DEPLOYMENT_PQSIG)) {
        scriptVerifyFlags |= SCRIPT_VERIFY_PQSIG;
    }
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // Blazecoin: SCRIPT_VERIFY_PQSIG is a hard-fork flag (it ADDS validity), so the cross-check must use the
    // next block's PQ state, not the tip's; otherwise every P2PQH spend accepted for block H_Q would trip
    // the "BUG! PLEASE REPORT" path while the tip is still H_Q - 1.
    if (DeploymentActiveAfter(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, Consensus::DEPLOYMENT_PQSIG)) {
        currentBlockScriptVerifyFlags |= SCRIPT_VERIFY_PQSIG;
    }
...
{
    // Blazecoin block reward schedule (values are consensus params; defaults in
    // consensus/params.h match the legacy chain):
    //   height == premineHeight (1):       premineSubsidy (20,650,000 BLZ)
    //   height <  earlySubsidyHeight (100): earlySubsidy   (13 BLZ)
    //   otherwise:                          baseSubsidy    (413 BLZ)
    // then halved every nSubsidyHalvingInterval. Block 0 is the genesis coinbase
    // (50 BLZ, hardcoded in CreateGenesisBlock) and is never queried here.
    CAmount nSubsidy;
    if (nHeight == consensusParams.premineHeight) {
        nSubsidy = consensusParams.premineSubsidy;
    } else if (nHeight < consensusParams.earlySubsidyHeight) {
        nSubsidy = consensusParams.earlySubsidy;
    } else {
        nSubsidy = consensusParams.baseSubsidy;
    }
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Blazecoin: P2PQH / OP_CHECKPQSIG from H_Q (PQ_SIGNATURES.md s7) - a hard fork by flag height.
    // DeploymentActiveAt(block) == DeploymentActiveAfter(block.pprev): the block at height H_Q itself
    // is the first validated with the flag.
    if (DeploymentActiveAt(block_index, chainman, Consensus::DEPLOYMENT_PQSIG)) {
    ...
    if (!m_chainman.AssumedValidBlock().IsNull()) {
        ...
        if (it != m_blockman.m_block_index.end()) {
            if (it->second.GetAncestor(pindex->nHeight) == pindex &&
                ...
                // Blazecoin: 30s blocks make the upstream 2-week equivalent-time gate
                // (sized for Bitcoin's 10-min spacing) trip during normal IBD whenever the
                // chain tip is more than ~40k blocks behind the header tip. Since our
                // defaultAssumeValid hash is hardcoded in source and the nMinimumChainWork
                // floor + always-on PoW check above already provide the security bounds,
                // unconditionally skip script verification once we're inside the ancestry
                // of the assumed-valid block.
                fScriptChecks = false;
            }
        }
    }

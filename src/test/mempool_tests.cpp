...
{
public:
    using CTxMemPool::CTxMemPool; // inherit ctors so tests can build a pool with custom Options
...
{
    // Blazecoin: the default incremental relay feerate is now 0 (D3 free-send fix in
    // policy.h). This test exercises the rolling-minimum-fee decay whose floor IS the
    // incremental relay feerate, so build a local pool pinned to the historical 1000
    // value — otherwise the "+incremental" and "never drop below 1000" assertions below
    // collapse to 0 and stop covering the decay/floor behaviour.
    auto mempool_opts = MemPoolOptionsForTest(m_node);
    mempool_opts.incremental_relay_feerate = CFeeRate(1000);
    bilingual_str mempool_error;
    MemPoolTest pool{mempool_opts, mempool_error};

...
BOOST_AUTO_TEST_CASE(create_directories)
{
    ...
    try {
        ...
        // Blazecoin/Windows: creating a symlink needs SeCreateSymbolicLinkPrivilege (admin or
        // Developer Mode). When it isn't held, skip the symlink half of this test rather than
        // letting the uncaught filesystem_error abort the entire test binary (which otherwise
        // cuts every later suite short). The create_directories() workaround above — the actual
        // point of this test — is already asserted; unaffected on Linux/macOS/privileged Windows.
        BOOST_WARN_MESSAGE(false, std::string{"fs_tests: skipping symlink portion — "} + e.what());
        fs::remove(dir);
        return;
    }

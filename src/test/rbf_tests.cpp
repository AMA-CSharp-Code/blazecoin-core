...
BOOST_FIXTURE_TEST_CASE(rbf_helper_functions, TestChain100Setup)
{
    ...
    // Blazecoin sets DEFAULT_INCREMENTAL_RELAY_FEE to 0 (free-send, see policy.h / audit D3), but
    // PaysForRBF's per-vsize increment math must still be exercised with a NON-zero increment — the
    // assertions below assume a 1 sat/vByte increment (1000 sat/kvB), so pin it explicitly here
    // instead of reading the (now 0) default.
    const CFeeRate incremental_relay_feerate{1000};
    const CFeeRate higher_relay_feerate{2 * 1000};
    ...
    // Tx4 has 23 descendants (and tx3 has 24: tx4 plus tx4's 23). Both entries violate the
    // cluster-size-2 rule, and CheckConflictTopology reports the FIRST violating entry in the
    // set's iteration order — which is txid order (CompareIteratorByHash). Upstream hardcoded
    // tx4 because its txids happened to sort that way; Blazecoin's deterministic test chain has
    // different coinbase txids, so the order can flip. Expect whichever entry sorts first.
    const auto& first_34{*set_34_cpfp.begin()};
    const auto expected_topology_err{first_34 == entry3_low
        ? strprintf("%s has 24 descendants, max 1 allowed", entry3_low->GetSharedTx()->GetHash().ToString())
        : strprintf("%s has 23 descendants, max 1 allowed", entry4_high->GetSharedTx()->GetHash().ToString())};
    BOOST_CHECK_EQUAL(pool.CheckConflictTopology(set_34_cpfp).value(), expected_topology_err);

...
#include <chain.h>
#include <consensus/consensus.h> // COINBASE_MATURITY (Blazecoin: 30 = TestChain100Setup height)
...
BOOST_FIXTURE_TEST_CASE(get_prune_height, TestChain100Setup)
{
    ...
    // Fresh chain of COINBASE_MATURITY (Blazecoin: 30) blocks — TestChain100Setup mines
    // COINBASE_MATURITY here, not the upstream 100 — with no pruned blocks, so nullopt.
    BOOST_CHECK(!GetPruneHeight(blockman, chain).has_value());

    // Start pruning
    CheckGetPruneHeight(blockman, chain, 1);
    CheckGetPruneHeight(blockman, chain, 15);
    CheckGetPruneHeight(blockman, chain, COINBASE_MATURITY);
}

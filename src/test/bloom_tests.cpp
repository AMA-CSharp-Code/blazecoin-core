...
{
    // Blazecoin changed the WIF (SECRET_KEY) base58 prefix, so the upstream Bitcoin WIF
    // "6CDNGoqZjQFDEGp2xtQQqkFZUzmmVFKh3Lyve6SVQSFXja7Ls3N" no longer decodes (invalid key ->
    // GetPubKey asserts). Set the SAME private key from raw hex (network-independent, uncompressed),
    // so the pubkey and the expected filter bytes below are unchanged.
    const auto vchSecret = ParseHex("f49addfd726a59abde172c86452f5f73038a02f4415878dc14934175e8418aff");
    CKey key;
...
BOOST_AUTO_TEST_CASE(merkle_block_2)
{
    ...
    // Match an output from the second transaction (the pubkey for address BgM9bPvmvuPtiK3B7DrGg6bonwTCfFVMa4)

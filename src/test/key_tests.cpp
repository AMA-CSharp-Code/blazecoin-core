...
static const std::string strSecret1 = "6AVsWxLjkm7i3svmcxSjjSFxxhEpdK3DkHjYudAbWoRoLz4XL3j";
static const std::string strSecret2 = "6BjREmX3tyaxXaPGx7G5HYbqqLZGydfRtypiMgAGrXSJnSRznfP";
static const std::string strSecret1C = "PnvRyDG7i9icBhnKwhibcJV3zpc1DGcoG4roqGdMLXvTZ5pETGyY";
static const std::string strSecret2C = "PtNDymVdnQkt5xMKQEdXv2UnYMKTWxpctRobYFqvybLhV98UsGmb";
...
/* Blazecoin: V1.5 / 0.8.x wallets store EC private keys in the compact named-curve form,
 * whose DER SEQUENCE uses SHORT-FORM length (< 0x80). key.cpp's ec_seckey_import_der was
 * patched to accept that (classic explicit-parameter keys use long form). CKey::Load
 * exercises that importer, so this guards the V1.5-wallet-import fix. */
BOOST_AUTO_TEST_CASE(blazecoin_import_short_form_der)
{
    CKey key;
    ...
    // Short-form-length ECPrivateKey: version + 32-byte key + secp256k1 OID params.
    //   30 2e 02 01 01 04 20 <32B secret> a0 07 06 05 2b 81 04 00 0a
    CPrivKey der;
    const unsigned char prefix[] = {0x30, 0x2e, 0x02, 0x01, 0x01, 0x04, 0x20};
    der.insert(der.end(), prefix, prefix + sizeof(prefix));
    der.insert(der.end(), UCharCast(key.begin()), UCharCast(key.end())); // 32-byte secret
    const unsigned char params[] = {0xa0, 0x07, 0x06, 0x05, 0x2b, 0x81, 0x04, 0x00, 0x0a};
    der.insert(der.end(), params, params + sizeof(params));
    BOOST_REQUIRE_EQUAL(der.size(), 48u);
    BOOST_CHECK(der[1] < 0x80); // short-form SEQUENCE length

    // Load must succeed AND (fSkipCheck=false) re-derive the same pubkey from the imported key.
    CKey loaded;
    BOOST_CHECK(loaded.Load(der, pubkey, /*fSkipCheck=*/false));
    BOOST_CHECK(loaded.IsValid());
    BOOST_CHECK(loaded == key);
}

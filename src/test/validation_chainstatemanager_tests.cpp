...
BOOST_FIXTURE_TEST_CASE(chainstatemanager, TestChain100Setup)
{
    ...
    // Blazecoin: the fixture mines COINBASE_MATURITY (30) blocks, not 100, so the assumeutxo
    // anchor height is 40 (30 + 10), matching the regtest m_assumeutxo_data entry.
    mineBlocks(10);
    BOOST_CHECK_EQUAL(WITH_LOCK(manager.GetMutex(), return manager.ActiveHeight()), 40);
    ...
    BOOST_CHECK_EQUAL(WITH_LOCK(manager.GetMutex(), return manager.ActiveHeight()), 40);
    mineBlocks(1);
    BOOST_CHECK_EQUAL(WITH_LOCK(manager.GetMutex(), return manager.ActiveHeight()), 41);
    BOOST_CHECK_EQUAL(WITH_LOCK(manager.GetMutex(), return c1.m_chain.Height()), 40);
...
struct SnapshotTestSetup : TestChain100Setup {
    ...
    std::tuple<Chainstate*, Chainstate*> SetupSnapshot()
    {
        ...
        size_t initial_size;
        // Blazecoin: the fixture mines COINBASE_MATURITY (30) blocks, not upstream's 100.
        size_t initial_total_coins{30};
        ...
        // Mine 10 more blocks, putting at us height 40 (Blazecoin: 30-block fixture + 10)
        // where a valid assumeutxo value can be found in the regtest chainparams.
        constexpr int snapshot_height = 40;
        mineBlocks(10);
        initial_size += 10;
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_loadblockindex, TestChain100Setup)
{
    ...
    // Blazecoin: heights remapped for the 30-block fixture (upstream: 111/120 on a 100-block
    // chain); the regtest assumeutxo anchor is at height 40.
    const int expected_assumed_valid{20};
    const int last_assumed_valid_idx{41};
    const int assumed_valid_start_idx = last_assumed_valid_idx - expected_assumed_valid;

    // Mine to height 50, past the hardcoded regtest assumeutxo snapshot at
    // height 40
    mineBlocks(20);
    ...
    BOOST_CHECK_EQUAL(assumed_tip->nHeight, 50);
    ...
    for (int i = 0; i <= cs1.m_chain.Height(); ++i) {
        ...
        // Blocks with heights in range [21, 40] are marked as missing data.
    ...
    BOOST_CHECK_EQUAL(num_indexes, 51); // 51 total blocks, including genesis
    BOOST_CHECK_EQUAL(assumed_tip->nHeight, 50);  // original chain has height 50 (Blazecoin: 30-block fixture + 20)
    BOOST_CHECK_EQUAL(validated_tip->nHeight, 20); // current cs1 chain has height 20
    BOOST_CHECK_EQUAL(assumed_base->nHeight, 40); // current cs2 chain has height 40
    ...
    // the assumed valid base as candidates, blocks 20 and 40. Specifically:
    //
    // - It does not have blocks 0-19 because they contain less work than the
    //   chain tip.
    //
    // - It has block 20 because it has data and equal work to the chain tip,
    //   (since it is the chain tip).
    //
    // - It does not have blocks 21-39 because they do not contain data.
    //
    // - It has block 40 even though it does not have data, because
    ...
    //
    // - It does not have any blocks after height 40 because cs1 is a background
    ...
    //
    // Specifically:
    // - All blocks below height 40 are not candidates, because cs2 chain tip
    //   has height 40 and they have less work than it does.
    //
    // - Block 40 is a candidate even though it does not have data, because it
    //   is the snapshot block, which is assumed valid.
    //
    // - Blocks 41-50 are added because they have data.

    // Check that block 20 is absent
    BOOST_CHECK_EQUAL(cs2.setBlockIndexCandidates.count(validated_tip), 0);
    // Check that block 39 is absent
    BOOST_CHECK_EQUAL(cs2.setBlockIndexCandidates.count(assumed_base->pprev), 0);
    // Check that block 40 is present
    BOOST_CHECK_EQUAL(cs2.setBlockIndexCandidates.count(assumed_base), 1);
    // Check that block 50 is present
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_init, SnapshotTestSetup)
{
    ...
    BOOST_CHECK_EQUAL(bg_chainstate.m_chain.Height(), 39); // Blazecoin: snapshot base (40) - 1
...

    {
        ...
        BOOST_CHECK_EQUAL(chainman_restarted.ActiveHeight(), 140); // Blazecoin: 40 + 100 mined post-snapshot
    }
    ...
    {
        LOCK(chainman_restarted.GetMutex());
        BOOST_CHECK_EQUAL(chainman_restarted.ActiveHeight(), 150); // Blazecoin: +10 more

...
BOOST_AUTO_TEST_CASE(stale_tip_peer_management)
{
    ...
    // TipMayBeStale() considers the tip stale after nPowTargetSpacing * 60 (Blazecoin changed this
    // from Bitcoin's * 3 so a slow 30s-block chain isn't flagged stale after a normal inter-block gap).
    // Advance past that (1800s at 30s spacing), which also clears the 10-min STALE_CHECK_INTERVAL gate.
    const auto time_later{time_init + 60 * std::chrono::seconds{m_node.chainman->GetConsensus().nPowTargetSpacing} + 1s};

...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    {
        // Blazecoin mainnet P2P port is 55414 (not Bitcoin's 8333); the connection log
        // reflects Params().GetDefaultPort(). A stale ":8333" here makes ASSERT_DEBUG_LOG
        // miss, and DebugLogHelper's destructor aborts the whole test binary.
        ASSERT_DEBUG_LOG("Added connection to 127.0.0.1:55414 peer=1");
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG("Not opening a connection to localhost, already connected to 127.0.0.1:55414");

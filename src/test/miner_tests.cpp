...
//
// Blazecoin: DISABLED. This upstream test is not portable to Blazecoin's consensus
// without a full economic rewrite, for three independent reasons:
//   1. PoW: the hardcoded BLOCKINFO nonces were pre-mined against Bitcoin's sha256d
//      regtest/mainnet target. Blazecoin validates PoW with scrypt GetPoWHash(). On the
//      MinerTestingSetup default chain (ChainType::MAIN) diff-1 is 0x1e0ffff0 (~1M scrypt
//      hashes/block, ~9h to regrind all 110); on regtest the nonces grind in ~2 iters but:
//   2. Subsidy: the test hardcodes Bitcoin's flat 50-coin reward (BLOCKSUBSIDY / the
//      5000000000LL literals) and reuses a single template's coinbase value for all 110
//      blocks. Blazecoin has a premine + ramped schedule (GetBlockSubsidy: height 1 =
//      20,650,000 BLZ, then earlySubsidy, then baseSubsidy, then halvings), so the reused
//      value trips bad-cb-amount and no single BLOCKSUBSIDY constant matches the coinbases
//      that TestBasicMining/TestPackageSelection/TestPrioritisedMining spend.
//   3. Deployments: the sequence-lock section assumes BIP68/CSV is not yet active at low
//      heights (true on Bitcoin mainnet, buried at 419328); on regtest CSV is always-active.
// The block-template / mining / sequence-lock logic this exercises IS covered for Blazecoin
// by the TestChain100Setup fixture (mines 30 real scrypt regtest blocks through CreateNewBlock
// + full validation; used across dozens of unit tests) and by the functional suite
// (test/functional/feature_blazecoin_*.py, which mines real scrypt blocks and validates
// templates end-to-end). Re-enabling requires a keyboard-present economic rewrite — see
// BLAZECOIN_DESIGN_AUDIT.md (D4).

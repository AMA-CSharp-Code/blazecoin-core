...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    const int attach_height{WITH_LOCK(::cs_main, return m_node.chainman->m_blockman.LookupBlockIndex(prev_hash)->nHeight + 1)};
    txCoinbase.vin[0].scriptSig = CScript{} << attach_height << OP_0;
    // Blazecoin: the template above was assembled against the ACTIVE tip, but this block is
    // attached at prev_hash (BuildChain builds whole fork trees before anything is processed,
    // so the active tip is still genesis). Bitcoin's flat regtest subsidy made that height
    // mismatch harmless; Blazecoin's is height-dependent (20,650,000-BLZ premine at height 1,
    // then 13 BLZ), so the templated reward — the height-1 premine — trips bad-cb-amount at
    // every deeper height. Re-derive the reward for the actual attach height (fees are
    // forfeited, which is always valid).
    txCoinbase.vout[1].nValue = GetBlockSubsidy(attach_height, Params().GetConsensus());

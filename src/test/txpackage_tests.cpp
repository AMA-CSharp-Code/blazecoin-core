...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    // Blazecoin: output amounts are derived from the spent coinbase's actual value (upstream
    // hardcoded 49/48 coins against Bitcoin's flat 50-coin subsidy). m_coinbase_txns[0] is the
    // block-1 premine (20,650,000 BLZ) and the rest are 13 BLZ, so each hop pays exactly
    // 1 COIN in fees — which the effective-feerate assertions below rely on.
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/mtx_parent.vout[0].nValue - COIN, /*submit=*/false);
...
BOOST_FIXTURE_TEST_CASE(noncontextual_package_tests, TestChain100Setup)
{
    ...
    // Parent and Child Package
    {
        // Blazecoin: amounts derived from the actual coinbase value (see package_validation_tests).
        auto mtx_parent = CreateValidMempoolTransaction(m_coinbase_txns[0], 0, 0, coinbaseKey, spk,
                                                        m_coinbase_txns[0]->vout[0].nValue - COIN, /*submit=*/false);
        CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);

        auto mtx_child = CreateValidMempoolTransaction(tx_parent, 0, 101, placeholder_key, spk2,
                                                       mtx_parent.vout[0].nValue - COIN, /*submit=*/false);
        CTransactionRef tx_child = MakeTransactionRef(mtx_child);
    ...
    // 24 Parents and 1 Child
    {
        ...
        // Blazecoin: parents spend 13-BLZ coinbases (not Bitcoin's 50), each with a 1-COIN fee;
        // the child's output is the sum of its parents' outputs minus a 1-COIN fee.
        CAmount child_input_value{0};
        for (int i{0}; i < 24; ++i) {
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, m_coinbase_txns[i + 1]->vout[0].nValue - COIN, false));
            package.emplace_back(parent);
            child.vin.emplace_back(COutPoint(parent->GetHash(), 0));
            child_input_value += parent->vout[0].nValue;
        }
        child.vout.emplace_back(child_input_value - COIN, spk2);
...
// Blazecoin: re-enabled 2026-07-06 after economic adaptation. The upstream version segfaulted
// here for two Blazecoin-specific reasons: (1) m_coinbase_txns[i + 25] indexed past the 30-entry
// coinbase vector (this fork's TestChain100Setup mines COINBASE_MATURITY = 30 blocks, not 100) —
// an out-of-bounds read; (2) hardcoded 49/48/47-coin outputs against Bitcoin's flat 50-coin
// subsidy, where Blazecoin's coinbases are the 20,650,000-BLZ premine ([0]) and 13 BLZ ([1..]).
// Amounts are now derived from the actual spent values (1-COIN fee per hop, preserving the
// CFeeRate(1 * COIN, vsize) package-feerate assertions below).
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    for (size_t i{0}; i < 10; ++i) {
        auto mtx = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[i + 1], /*input_vout=*/0,
                                                 /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                 /*output_destination=*/parent_locking_script,
                                                 /*output_amount=*/m_coinbase_txns[i + 1]->vout[0].nValue - COIN, /*submit=*/false);
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/tx_child, /*input_vout=*/0,
                                                       ...
                                                       /*output_amount=*/mtx_child.vout[0].nValue - COIN, /*submit=*/false);
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    // Blazecoin: amounts derived from the actual coinbase values (premine at [0], 13 BLZ after).
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
    ...
    mtx_child1.vout[0].nValue = mtx_parent.vout[0].nValue - COIN;
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/ptx_child2, /*input_vout=*/0,
                                                        ...
                                                        /*output_destination=*/grandchild_locking_script,
                                                        /*output_amount=*/ptx_child2->vout[0].nValue - COIN, /*submit=*/false);
    ...
    auto mtx_parent1 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                     ...
                                                     /*output_amount=*/m_coinbase_txns[1]->vout[0].nValue - COIN, /*submit=*/true);
    ...
    auto mtx_grandparent2 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                          ...
                                                          /*output_amount=*/m_coinbase_txns[2]->vout[0].nValue - COIN, /*submit=*/true);
    ...
    mtx_parent2_v1.vout[0].nValue = mtx_grandparent2.vout[0].nValue - COIN;
    ...
    auto mtx_parent3 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[3], /*input_vout=*/0,
                                                     /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                     /*output_destination=*/acs_spk,
                                                     /*output_amount=*/m_coinbase_txns[3]->vout[0].nValue - low_fee_amt, /*submit=*/false);
    ...
    // Blazecoin: child fee = COIN - low_fee_amt, so parent3 (fee low_fee_amt) + child sum to
    // exactly 1 COIN — the expected package feerate asserted below. (Upstream's
    // (48+49+50-1)*COIN encoded the same relationship for 50-coin coinbases.)
    const CAmount mixed_child_inputs{ptx_parent1->vout[0].nValue + ptx_parent2_v1->vout[0].nValue + ptx_parent3->vout[0].nValue};
    mtx_mixed_child.vout.emplace_back(mixed_child_inputs - (COIN - low_fee_amt), mixed_child_spk);
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    // Blazecoin: per-coinbase values instead of upstream's flat 50 * COIN — [0] is the
    // 20,650,000-BLZ premine; [1] and [2] are 13-BLZ blocks. All fee relationships below are
    // differences from these, so they carry over unchanged.
    const CAmount coinbase_value{m_coinbase_txns[0]->vout[0].nValue};
    const CAmount coinbase_value_cheap{m_coinbase_txns[1]->vout[0].nValue};
    const CAmount coinbase_value_rich{m_coinbase_txns[2]->vout[0].nValue};
    ...
    auto mtx_parent_cheap = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                          /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                          /*output_destination=*/parent_spk,
                                                          /*output_amount=*/coinbase_value_cheap - parent_fee, /*submit=*/false);
    ...
    auto mtx_child_cheap = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent_cheap, /*input_vout=*/0,
                                                         /*input_height=*/101, /*input_signing_key=*/child_key,
                                                         /*output_destination=*/child_spk,
                                                         /*output_amount=*/coinbase_value_cheap - parent_fee - child_fee, /*submit=*/false);
    ...
    auto mtx_parent_rich = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                         /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                         /*output_destination=*/parent_spk,
                                                         /*output_amount=*/coinbase_value_rich - high_parent_fee, /*submit=*/false);
    ...
    // Parent pays 1 BTC and child pays none. The parent should be accepted without the child.
    {
        ...
        if (auto err_rich_parent{CheckPackageMempoolAcceptResult(package_rich_parent, submit_rich_parent, /*expect_valid=*/false, m_node.mempool.get())}) {
            ...
            // Blazecoin: the min relay fee is 0 (free-send, see DEFAULT_MIN_RELAY_TX_FEE /
            // DEFAULT_INCREMENTAL_RELAY_FEE in policy.h), so upstream's expected
            // TX_MEMPOOL_POLICY / "min relay fee not met" can never fire for this 0-fee child —
            // it passes the relay-fee check and instead fails the mocked dynamic mempool minimum
            // feerate (MockMempoolMinFee(5000) above) as TX_RECONSIDERABLE.
...
BOOST_FIXTURE_TEST_CASE(package_rbf_tests, TestChain100Setup)
{
    ...
    // Blazecoin: per-coinbase values ([0] = 20,650,000-BLZ premine, [1] = 13 BLZ) instead of
    // upstream's flat 50 * COIN. All fees below are differences from these, so they carry over.
    const CAmount coinbase_value{m_coinbase_txns[0]->vout[0].nValue};
    const CAmount coinbase_value_1{m_coinbase_txns[1]->vout[0].nValue};

...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    // Blazecoin: MAX_MONEY is 2,065,000,000 BLZ (not Bitcoin's 21M), so upstream's
    // 100M-coin vector is a perfectly representable amount here; pin the exact cap as
    // parseable and probe just past it instead.
    BOOST_CHECK_EQUAL(ParseMoney("2065000000.00").value(), MAX_MONEY);
    BOOST_CHECK(!ParseMoney("2065000000.00000001"));
    BOOST_CHECK(!ParseMoney("2100000000.00"));
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        // derived address from this private key: BXz7Z5PTtWKLbmnBmCSaXkiN67KrXaQ3Qj
    ...
    // Blazecoin: MESSAGE_MAGIC is "Blazecoin Signed Message:\n" (signmessage.cpp), so the
    // upstream signature (made under Bitcoin's magic) can never match. This is the
    // deterministic (RFC 6979) signature of `message` under the Blazecoin magic with the
    // private key above — regenerated 2026-07-06 and pinned as a regression vector.
    const std::string expected_signature =
        "H5q4saRHH12Aa3368eWJqgtSjuqyrWV1Uyk6JSLmLnm3PEInY7Ipdq238L05kJfYebHR3XB99WmUekB98qr0p8g=";
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "BndGne6TXwDD2j1ThQE4pUu16sBTENUzsF",
    ...
    // Blazecoin: signature regenerated under the Blazecoin MESSAGE_MAGIC (see message_sign).
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "BXz7Z5PTtWKLbmnBmCSaXkiN67KrXaQ3Qj",
            "H5q4saRHH12Aa3368eWJqgtSjuqyrWV1Uyk6JSLmLnm3PEInY7Ipdq238L05kJfYebHR3XB99WmUekB98qr0p8g=",
            "I never signed this"),
    ...
    // Blazecoin: upstream had a second OK vector here ("Trust me" against
    // BToJBcfCGqZoe5FBqjnEz7UzptBD1Y3BgV with a signature whose r-value was nonce-ground to
    // spell "! Chancellor on brink of second bailout for banks" in base64). That signature
    // was crafted under Bitcoin's message magic with a one-off grinding setup and cannot be
    // regenerated for the Blazecoin magic without the original private key; the OK path is
    // already covered by the vector above, so it is dropped rather than faked.
}
...
BOOST_AUTO_TEST_CASE(message_sign_verify_pq_blazecoin)
{
    // Blazecoin P2PQH message signatures (PQ_SIGNATURES.md s8.3):
    //   digest = TaggedHash("Blazecoin/PQMsg/v1", MessageHash(message)), ctx "blazecoin-msg-v1",
    //   signature = base64(keyblob || sig) = 3,733 bytes, verifiable against the BQ address alone.
    namespace V = pqsig_vectors;
    const auto master = ParseHex(V::DERIVATION[0].master_seed);
    std::vector<unsigned char> pk, sk, pk1, sk1;
    BOOST_REQUIRE(mldsa44::KeyGenFromSeed(pq::PQSeedForIndex(master, 0), pk, sk));
    BOOST_REQUIRE(mldsa44::KeyGenFromSeed(pq::PQSeedForIndex(master, 1), pk1, sk1));
    BOOST_CHECK_EQUAL(HexStr(pk), V::DERIVATION[0].pubkey);
    const std::string address = V::DERIVATION[0].address_main; // this suite runs on mainnet params
    const std::string other = V::DERIVATION[1].address_main;
    const std::string message = "Trust no one, verify the pqkh";

    // The digest definition, spelled out.
    {
        HashWriter h{TaggedHash("Blazecoin/PQMsg/v1")};
        h << MessageHash(message);
        BOOST_CHECK(PQMessageDigest(message) == h.GetSHA256());
        BOOST_CHECK_EQUAL(PQ_MESSAGE_TAG, "Blazecoin/PQMsg/v1");
        BOOST_CHECK_EQUAL(PQ_MESSAGE_CTX, "blazecoin-msg-v1");
    }

    std::string signature;
    BOOST_REQUIRE(MessageSignPQ(pk, sk, message, signature));
    const auto bytes = DecodeBase64(signature);
    BOOST_REQUIRE(bytes);
    BOOST_REQUIRE_EQUAL(bytes->size(), 1313U + 2420U);
    BOOST_CHECK_EQUAL((*bytes)[0], pq::PQ_ALGO_MLDSA44);
    BOOST_CHECK(std::equal(pk.begin(), pk.end(), bytes->begin() + 1));
    {
        // Encoding is exactly keyblob || sig, under the message context (domain-separated from tx signing).
        const Span<const unsigned char> sig{bytes->data() + 1313, 2420};
        const std::string ctx_msg{"blazecoin-msg-v1"}, ctx_tx{"blazecoin-tx-v1"};
        const auto span = [](const std::string& s) { return Span<const unsigned char>{reinterpret_cast<const unsigned char*>(s.data()), s.size()}; };
        BOOST_CHECK(mldsa44::Verify(pk, PQMessageDigest(message), span(ctx_msg), sig));
        BOOST_CHECK(!mldsa44::Verify(pk, PQMessageDigest(message), span(ctx_tx), sig));
        BOOST_CHECK(!mldsa44::Verify(pk, MessageHash(message), span(ctx_msg), sig));
    }
    // Deterministic.
    std::string again;
    BOOST_REQUIRE(MessageSignPQ(pk, sk, message, again));
    BOOST_CHECK_EQUAL(again, signature);

    BOOST_CHECK(MessageVerify(address, signature, message) == MessageVerificationResult::OK);
    BOOST_CHECK(MessageVerify(address, signature, message + " ") == MessageVerificationResult::ERR_NOT_SIGNED);
    BOOST_CHECK(MessageVerify(other, signature, message) == MessageVerificationResult::ERR_NOT_SIGNED);
    BOOST_CHECK(MessageVerify("BQinvalid", signature, message) == MessageVerificationResult::ERR_INVALID_ADDRESS);
    BOOST_CHECK(MessageVerify(address, "not base64!", message) == MessageVerificationResult::ERR_MALFORMED_SIGNATURE);
    BOOST_CHECK(MessageVerify(address, EncodeBase64(Span{*bytes}.first(bytes->size() - 1)), message) == MessageVerificationResult::ERR_MALFORMED_SIGNATURE);
    {
        std::vector<unsigned char> flipped = *bytes;
        flipped.back() ^= 0x01; // signature byte
        BOOST_CHECK(MessageVerify(address, EncodeBase64(flipped), message) == MessageVerificationResult::ERR_NOT_SIGNED);
        std::vector<unsigned char> wrong_algo = *bytes;
        wrong_algo[0] = pq::PQ_ALGO_MLDSA65_RESERVED;
        BOOST_CHECK(MessageVerify(address, EncodeBase64(wrong_algo), message) == MessageVerificationResult::ERR_MALFORMED_SIGNATURE);
        std::vector<unsigned char> swapped_key = *bytes;
        std::copy(pk1.begin(), pk1.end(), swapped_key.begin() + 1); // another key's blob does not hash to the address
        BOOST_CHECK(MessageVerify(address, EncodeBase64(swapped_key), message) == MessageVerificationResult::ERR_NOT_SIGNED);
    }
    // A legacy (65-byte compact ECDSA) signature is malformed for a BQ address, and a PQ signature is not
    // a recoverable ECDSA one for a legacy address.
    {
        CKey privkey;
        privkey.MakeNewKey(true);
        std::string legacy_sig;
        BOOST_REQUIRE(MessageSign(privkey, message, legacy_sig));
        BOOST_CHECK(MessageVerify(address, legacy_sig, message) == MessageVerificationResult::ERR_MALFORMED_SIGNATURE);
        const std::string legacy_addr = EncodeDestination(PKHash(privkey.GetPubKey()));
        BOOST_CHECK(MessageVerify(legacy_addr, signature, message) == MessageVerificationResult::ERR_PUBKEY_NOT_RECOVERED);
    }
    // Sizes are enforced on the signing side too.
    BOOST_CHECK(!MessageSignPQ(pk, Span{sk}.first(100), message, signature));
    BOOST_CHECK(!MessageSignPQ(Span{pk}.first(100), sk, message, signature));
}

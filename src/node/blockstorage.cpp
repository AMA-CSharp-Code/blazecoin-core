...
bool BlockTreeDB::LoadBlockIndexGuts(const Consensus::Params& consensusParams, std::function<CBlockIndex*(const uint256&)> insertBlockIndex, const util::SignalInterrupt& interrupt)
{
    ...
    // Defense-in-depth: after loading, re-verify scrypt PoW for only the most recent
    // (highest-height) blocks — see the note in the loop and the verify pass below.
    // Keep the top-N pointers in a bounded min-heap (smallest height on top, so it's
    // the one we evict) instead of a second DB pass over the whole index.
    static constexpr int RECENT_POW_RECHECK{2016};
    const auto by_height_desc = [](const CBlockIndex* a, const CBlockIndex* b) { return a->nHeight > b->nHeight; };
    std::priority_queue<CBlockIndex*, std::vector<CBlockIndex*>, decltype(by_height_desc)> recent(by_height_desc);
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // Blazecoin: do NOT re-verify scrypt PoW for every index entry here.
                // Stock Bitcoin's equivalent check is cheap — it passes GetBlockHash(),
                // the cached SHA256d identity hash. Blazecoin's PoW is scrypt, so the
                // fork changed this to GetBlockHeader().GetPoWHash(), which RECOMPUTES the
                // scrypt hash (~0.2 ms/block). Over a multi-million-block index that made
                // every startup a ~15-minute scrypt grind (the entire "Loading block
                // index…" warmup). Every block here was already PoW-validated when first
                // accepted, and leveldb checksums catch on-disk corruption, so re-checking
                // the WHOLE chain's PoW on each load is redundant and is dropped.
                //
                // As cheap defense-in-depth against local tip-region index tampering, we
                // still re-verify PoW for just the most recent RECENT_POW_RECHECK blocks
                // (after the load, below). Keep the N highest-height entries as we go.
                if (recent.size() < static_cast<size_t>(RECENT_POW_RECHECK)) {
                    recent.push(pindexNew);
                } else if (pindexNew->nHeight > recent.top()->nHeight) {
                    recent.pop();
                    recent.push(pindexNew);
                }
    ...
    // Bounded tip-region PoW recheck (defense-in-depth; see the note in the loop).
    // ~RECENT_POW_RECHECK scrypt hashes — sub-second — vs the full-chain grind we
    // dropped. PoW for older blocks stays enforced at block accept + header sync.
    while (!recent.empty()) {
        if (interrupt) return false;
        const CBlockIndex* pindex = recent.top();
        recent.pop();
        if (!CheckProofOfWork(pindex->GetBlockHeader().GetPoWHash(), pindex->nBits, consensusParams)) {
            LogError("%s: CheckProofOfWork failed for recent block at height %d (%s)\n",

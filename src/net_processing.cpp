 ...
 *  At Blazecoin's 30 s spacing this is 300 s (5 min); upstream Bitcoin's "1" multiplier
 *  with 10 min spacing also yielded 10 min — the multiplier was scaled to keep the
 *  wall-clock window in the same order of magnitude. */
static constexpr double BLOCK_DOWNLOAD_TIMEOUT_BASE = 10;
/** Additional block download timeout per parallel downloading peer, in multiples of
 *  nPowTargetSpacing. At Blazecoin's 30 s spacing this is 150 s (2.5 min). */
static constexpr double BLOCK_DOWNLOAD_TIMEOUT_PER_PEER = 5;
...
bool PeerManagerImpl::TipMayBeStale()
{
    ...
    return m_last_tip_update.load() < GetTime<std::chrono::seconds>() - std::chrono::seconds{consensusParams.nPowTargetSpacing * 60} && mapBlocksInFlight.empty();
}

...
/** Blazecoin: build a "blz" proprietary entry whose key is serialized the way Unserialize reads it. */
static PSBTProprietary MakeBlzProprietary(uint8_t subtype, std::vector<unsigned char> value)
{
    PSBTProprietary prop;
    prop.identifier = PSBT_BLZ_IDENTIFIER;
    prop.subtype = subtype;
    prop.value = std::move(value);
    DataStream key;
    key << CompactSizeWriter(PSBT_IN_PROPRIETARY) << prop.identifier << CompactSizeWriter(subtype);
    prop.key.assign(UCharCast(key.data()), UCharCast(key.data()) + key.size());
    return prop;
}

static void EraseBlzProprietary(std::set<PSBTProprietary>& props)
{
    for (auto it = props.begin(); it != props.end();) {
        if (it->identifier == PSBT_BLZ_IDENTIFIER) {
            it = props.erase(it);
        } else {
            ++it;
        }
...
void PSBTInput::FillSignatureData(SignatureData& sigdata) const
{
    ...
    for (const auto& prop : m_proprietary) {
        if (prop.identifier != PSBT_BLZ_IDENTIFIER) continue;
        if (prop.subtype == PSBT_BLZ_PQ_SIG) sigdata.pq_sig = prop.value;
        if (prop.subtype == PSBT_BLZ_PQ_KEYBLOB) sigdata.pq_keyblob = prop.value;
    }
...
void PSBTInput::FromSignatureData(const SignatureData& sigdata)
{
    if (sigdata.complete) {
        ...
        EraseBlzProprietary(m_proprietary);
    ...
    if (!sigdata.pq_sig.empty() && !sigdata.pq_keyblob.empty()) {
        // Blazecoin P2PQH: an unfinalized ML-DSA-44 signature travels in the "blz" proprietary fields.
        EraseBlzProprietary(m_proprietary);
        m_proprietary.insert(MakeBlzProprietary(PSBT_BLZ_PQ_SIG, sigdata.pq_sig));
        m_proprietary.insert(MakeBlzProprietary(PSBT_BLZ_PQ_KEYBLOB, sigdata.pq_keyblob));
    }
...
bool PSBTInputSignedAndVerified(const PartiallySignedTransaction psbt, unsigned int input_index, const PrecomputedTransactionData* txdata)
{
    ...
    // Blazecoin: SCRIPT_VERIFY_PQSIG selects the P2PQH evaluation path for a 34-byte template prevout.
    constexpr unsigned int flags{STANDARD_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_PQSIG};
    if (txdata) {

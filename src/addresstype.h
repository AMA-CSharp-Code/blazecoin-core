...
/** Blazecoin P2PQH destination: the 32-byte pqkh = TaggedHash("Blazecoin/PQKH/v1", algo_id || pubkey).
 *  Address = Base58Check(PQ_ADDRESS prefix || pqkh) - "BQ..." on mainnet, "TQ..." on test networks
 *  (PQ_SIGNATURES.md s5). Script = 0x20 <pqkh> OP_CHECKPQSIG (s3.2). */
struct PQKeyHash : public BaseHash<uint256>
{
    PQKeyHash() : BaseHash() {}
    explicit PQKeyHash(const uint256& hash) : BaseHash(hash) {}
};
 ...
 *  * PQKeyHash: TxoutType::PQ_KEYHASH destination (Blazecoin P2PQH "BQ..." address)
 *  A CTxDestination is the internal data type encoded in a bitcoin address
 */
using CTxDestination = std::variant<CNoDestination, PubKeyDestination, PKHash, ScriptHash, WitnessV0ScriptHash, WitnessV0KeyHash, WitnessV1Taproot, PayToAnchor, WitnessUnknown, PQKeyHash>;

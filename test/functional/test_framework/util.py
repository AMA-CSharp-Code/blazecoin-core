...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "blazecoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)  # Blazecoin: rebranded conf filename
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "blazecoin.conf"), 'a', encoding='utf8') as f:  # Blazecoin: rebranded conf filename
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "blazecoin.conf")):  # Blazecoin: rebranded conf filename
        with open(os.path.join(datadir, "blazecoin.conf"), 'r', encoding='utf8') as f:
            for line in f:

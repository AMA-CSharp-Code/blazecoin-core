...
>   §4/§5a/§8 re-trued 2026-09-10 after the 2026-09-05 v2.0.1 public release.)*
>
...
- **Checkpoint:** ✅ DONE — `H_A` = 4,194,000 itself (`add35fb6…`) is the 17th
  mainnet checkpoint since 2026-08-26 (standard 500k cadence continues; the
  extra one at the fork is cheap insurance).
...
(historical artifact, frozen at `H_A` by design). Fork-day comms: ✅ the website
Wallet page carries the post-fork card set (Original = ARCHIVED, V1.5 links →
`v1.5.2-*`) since 2026-08-29 and the Bitcointalk lineage thread carries the
announcement; the V1.5 README notice ("run v1.5.2 or later — older builds stop at `H_A`")
was added 2026-09-10 (`Blazecoin_Core_V1.5/README.md`, top banner; it had been owed since fork day).

**🚦 RELEASE GATE (Andrew, 2026-08-26) — LIFTED.** The rule was: no GitHub
release, tag, or downloadable binary for V1.5.2 until EVERY wallet implementation
was done — V2 daemon, lite heads (WASM/Android/iOS as applicable), all V1.5
platforms — and `H_A` chosen. Outcome: **V1.5.2 released 2026-08-26** for
Windows / macOS / Linux (`v1.5.2-*` tags, see the status block above), and
**V2 published 2026-09-05** as kit `v2.0.1` (`github.com/AMA-CSharp-Code/blazecoin-core`
+ `blazecoin-wallet`, daemon identity 2.0.0) with `CLIENT_VERSION_IS_RELEASE`
`true`. Per-platform V1.5.2 build status as it stood on 2026-08-26 (kept as the
build record):
...
- **macOS** — ✅ released 2026-08-26 with the others (per the status block
  above). Pre-release note kept for the record: the `macos-v1.5.0` branch was
  unbuildable/untestable on this box, so the plan was to merge `31b9a1d` into
  the branch **on a Mac**, build the universal .app, test, then release together
...
1. ~~**`H_A` value** — after all heads ship + margin; 413-motif optional.~~
   **DECIDED 2026-08-26: `H_A` = 4,194,000** (chosen at tip 4,193,990 with the
   miner stopped; no 413 motif).

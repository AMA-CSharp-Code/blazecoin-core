> **Blazecoin note (added 2026-08-10):** everything below is the **verbatim
> upstream Bitcoin Core 28.0 release notes** — the base this fork was cut from.
> Nothing below describes Blazecoin: the URLs, ports, datadir names and
> contributor credits are Bitcoin's. For what actually changed on this chain
> (consensus deltas, policy, ports, deploy state), read **`BLAZECOIN_V2.md`**
> at the repo root — the authoritative fork document. The regenerated
> `doc/man/*.1` pages are likewise upstream-branded and not Blazecoin-specific.
